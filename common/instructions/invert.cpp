...
const std::string Invert::MNEMONIC = "INV";

Invert::Invert(uint16_t input_memory_address, uint16_t output_memory_address) :
    m_input_address(input_memory_address),
    m_output_address(output_memory_address)
{
}

void Invert::operator()(std::vector<int32_t>& memory, uint8_t& subprogram_index,
...
std::vector<std::string> Invert::toStringTokens() const
{
    return std::vector<std::string>{ MNEMONIC, std::to_string(m_input_address), std::to_string(m_output_address) };
}

std::unique_ptr<Instruction> Invert::fromStringTokens(const std::vector<std::string>& tokens)
{
    if (tokens.size() != 3) {
        return nullptr;
    }
    ...
    return std::make_unique<Invert>(input_memory_address, output_memory_address);
}

std::vector<uint8_t> Invert::toByteArray() const
{
    std::vector<uint8_t> array;
    array.push_back(static_cast<uint8_t>(Id::INVERT));
    Utils::insertByteArray(array, m_input_address);
    Utils::insertByteArray(array, m_output_address);
    return array;
}

std::unique_ptr<Instruction> Invert::fromByteArray(const std::vector<uint8_t>& array, size_t& offset)
{
    if (array.size() - offset < sizeof(uint8_t) + sizeof(uint16_t) * 2) {
        return nullptr;
    }

    if (array[offset++] != static_cast<uint8_t>(Id::INVERT)) {
        return nullptr;
    }
