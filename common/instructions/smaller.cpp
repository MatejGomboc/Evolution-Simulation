...
const std::string Smaller::MNEMONIC = "SMA";

Smaller::Smaller(uint16_t input1_memory_address, uint16_t input2_memory_address, uint16_t output_memory_address) :
...
void Smaller::operator()(std::vector<int32_t>& memory, uint8_t& subprogram_index,
...
std::vector<std::string> Smaller::toStringTokens() const
{
...
std::unique_ptr<Instruction> Smaller::fromStringTokens(const std::vector<std::string>& tokens)
{
    if (tokens.size() != 4) {
        return nullptr;
    }
    ...
    return std::make_unique<Smaller>(input1_memory_address, input2_memory_address, output_memory_address);
}

std::vector<uint8_t> Smaller::toByteArray() const
{
    std::vector<uint8_t> array;
    array.push_back(static_cast<uint8_t>(Id::SMALLER));
...
std::unique_ptr<Instruction> Smaller::fromByteArray(const std::vector<uint8_t>& array, size_t& offset)
{
    if (array.size() - offset < sizeof(uint8_t) + sizeof(uint16_t) * 3) {
        return nullptr;
    }

    if (array[offset++] != static_cast<uint8_t>(Id::SMALLER)) {
        return nullptr;
    }
