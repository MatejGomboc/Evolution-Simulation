...
Add::Add(uint16_t input1_memory_address, uint16_t input2_memory_address, uint16_t output_memory_address) :
...
std::unique_ptr<Instruction> Add::fromStringTokens(const std::vector<std::string>& tokens)
{
    ...
    return std::make_unique<Add>(input1_memory_address, input2_memory_address, output_memory_address);
}
