...
And::And(uint16_t input1_memory_address, uint16_t input2_memory_address, uint16_t output_memory_address) :
...
void And::operator()(std::vector<int32_t>& memory, uint8_t& subprogram_index,
...
std::unique_ptr<Instruction> And::fromStringTokens(const std::vector<std::string>& tokens)
{
    ...
    return std::make_unique<And>(input1_memory_address, input2_memory_address, output_memory_address);
}

std::vector<uint8_t> And::toByteArray() const
{
    std::vector<uint8_t> array;
    array.push_back(static_cast<uint8_t>(Id::AND));
...
std::unique_ptr<Instruction> And::fromByteArray(const std::vector<uint8_t>& array, size_t& offset)
{
    if (array.size() - offset < sizeof(uint8_t) + sizeof(uint16_t) * 3) {
        return nullptr;
    }

    if (array[offset++] != static_cast<uint8_t>(Id::AND)) {
        return nullptr;
    }
