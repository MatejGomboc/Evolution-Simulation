...
const std::string Clear::MNEMONIC = "CLR";

Clear::Clear(uint16_t input1_memory_address, uint16_t input2_memory_address, uint16_t output_memory_address) :
...
void Clear::operator()(std::vector<int32_t>& memory, uint8_t& subprogram_index,
...
std::vector<std::string> Clear::toStringTokens() const
{
...
std::unique_ptr<Instruction> Clear::fromStringTokens(const std::vector<std::string>& tokens)
{
    if (tokens.size() != 4) {
        return nullptr;
    }
    ...
    return std::make_unique<Clear>(input1_memory_address, input2_memory_address, output_memory_address);
}

std::vector<uint8_t> Clear::toByteArray() const
{
    std::vector<uint8_t> array;
    array.push_back(static_cast<uint8_t>(Id::CLEAR));
...
std::unique_ptr<Instruction> Clear::fromByteArray(const std::vector<uint8_t>& array, size_t& offset)
{
    if (array.size() - offset < sizeof(uint8_t) + sizeof(uint16_t) * 3) {
        return nullptr;
    }

    if (array[offset++] != static_cast<uint8_t>(Id::CLEAR)) {
        return nullptr;
    }
