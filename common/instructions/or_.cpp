...
Or::Or(uint16_t input1_memory_address, uint16_t input2_memory_address, uint16_t output_memory_address) :
...
void Or::operator()(std::vector<int32_t>& memory, uint8_t& subprogram_index,
...
std::unique_ptr<Instruction> Or::fromStringTokens(const std::vector<std::string>& tokens)
{
    ...
    return std::make_unique<Or>(input1_memory_address, input2_memory_address, output_memory_address);
}

std::vector<uint8_t> Or::toByteArray() const
{
    std::vector<uint8_t> array;
    array.push_back(static_cast<uint8_t>(Id::OR));
