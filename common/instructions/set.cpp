...
const std::string Set::MNEMONIC = "SET";

Set::Set(uint16_t input1_memory_address, uint16_t input2_memory_address, uint16_t output_memory_address) :
...
void Set::operator()(std::vector<int32_t>& memory, uint8_t& subprogram_index,
...
std::vector<std::string> Set::toStringTokens() const
{
...
std::unique_ptr<Instruction> Set::fromStringTokens(const std::vector<std::string>& tokens)
{
    if (tokens.size() != 4) {
        return nullptr;
    }
    ...
    return std::make_unique<Set>(input1_memory_address, input2_memory_address, output_memory_address);
}

std::vector<uint8_t> Set::toByteArray() const
{
    std::vector<uint8_t> array;
    array.push_back(static_cast<uint8_t>(Id::SET));
...
std::unique_ptr<Instruction> Set::fromByteArray(const std::vector<uint8_t>& array, size_t& offset)
{
    if (array.size() - offset < sizeof(uint8_t) + sizeof(uint16_t) * 3) {
        return nullptr;
    }

    if (array[offset++] != static_cast<uint8_t>(Id::SET)) {
        return nullptr;
    }
